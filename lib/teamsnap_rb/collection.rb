module TeamsnapRb
  class Collection
    ...
    def initialize(url, query_parameters, options = {})
    ...
    def this
      @this ||= Collection.new(this_href, {})
    end

    def links
      @links ||= LinksProxy.new(collection_json.links)
    end

    def queries
      @queries ||= QueriesProxy.new(collection_json.queries)
    end

    def template
      @template ||= TemplateProxy.new(collection_json.template, href)
    end
    ...
    attr_accessor :collection_json, :items, :url
    attr_writer :errors

    def get(url, query_parameters = {})
      RequestBuilder.new(url).connection.get do |conn|
